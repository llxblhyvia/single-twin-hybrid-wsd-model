...
parser.add_argument('--context-bsz', type=int, default=2)
...
gloss_device = "cuda:1"
uni_device = torch.device("cuda:4")
...
def preprocess_context(tokenizer, text_data, bsz=1, max_len=-1):
    ...
    for sent in text_data:
        c_ids = [
            torch.tensor([tokenizer.encode(tokenizer.cls_token)])]  # cls token aka sos token, returns a list with index
        o_masks = [-1]
        sent_insts = []
        sent_keys = []
        sent_labels = []
        ...
        o_masks.append(-1)
        c_ids, c_attn_masks, o_masks = normalize_length(c_ids, c_attn_mask, o_masks, max_len,
                                                        pad_id=tokenizer.encode(tokenizer.pad_token)[0])
...
def _train(train_data, model, gloss_dict, optim, schedule, criterion, gloss_bsz=-1, max_grad_norm=1.0, multigpu=False,
           silent=False, train_steps=-1):
    model.train()
    total_loss = 0.
    ...
    for i, (context_ids, context_attn_mask, context_output_mask, example_keys, _, labels) in train_data:
        ...
        if multigpu:
            ...
            '''
            context_ids = context_ids.to(uni_device)
            context_attn_mask = context_attn_mask.to(uni_device)
        ...
        for j, (key, label) in enumerate(zip(example_keys, labels)):
            ...
            ori_output = output
            ori_output = ori_output.to(uni_device)
            ...
            if multigpu:
                ...
                '''
                gloss_ids = gloss_ids.to(uni_device)
                gloss_attn_mask = gloss_attn_mask.to(uni_device)

            gloss_output = model.gloss_forward(gloss_ids, gloss_attn_mask)
            ori_gloss_output = gloss_output.transpose(0, 1)
            ori_gloss_output = ori_gloss_output.to(uni_device)
            #双塔loss
            ori_output = torch.mm(ori_output,ori_gloss_output)


            #单塔loss
            conca_output = torch.cat((output, gloss_output), 0).unsqueeze(1)
            conca_output = conca_output.to(uni_device)

            trans_output = torch.tensor([0])
            trans_output = trans_output.to(uni_device)
            for i in range(0, 2):
                if i == 0:
                    trans_layer = torch.nn.TransformerEncoderLayer(d_model=768, nhead=12)
                    trans_layer.to(uni_device)
                    trans_output = trans_layer(conca_output)
                else:
                    trans_layer = torch.nn.TransformerEncoderLayer(d_model=768, nhead=12)
                    trans_layer.to(uni_device)
                    trans_output = trans_layer(trans_output)

            output = trans_output[0]
            # output = output.to(uni_device)
            if trans_output.shape[0] < 3:
                gloss_output = trans_output[1]
            else:
                gloss_output = torch.squeeze(trans_output[1:])
    ...
    for context_ids, context_attn_mask, context_output_mask, example_keys, insts, _ in eval_data:
        with torch.no_grad():
            ...
            for output, key, inst in zip(context_output.split(1, dim=0), example_keys, insts):
                ...
                gloss_output = gloss_output.transpose(0, 1)
                gloss_output = gloss_output.to(uni_device)
...
def train_model(args):
    ...
    train_gloss_dict, train_gloss_weights = load_and_preprocess_glosses(train_data, tokenizer, wn_senses,
                                                                        max_len=args.gloss_max_length)
    semeval2007_gloss_dict, _ = load_and_preprocess_glosses(semeval2007_data, tokenizer, wn_senses,
                                                            max_len=args.gloss_max_length)
    ...
    for epoch in range(1, epochs + 1):
        ...
        model, optimizer, schedule, train_loss = _train(train_data, model, train_gloss_dict, optimizer, schedule,
                                                        criterion, gloss_bsz=args.gloss_bsz,
